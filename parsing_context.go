...
func (p *parsingContext) setValue() {
	if p.inObject() {
		p.key = p.path // reset key to parent
	} else {
		p.increaseElemsCount()
	}
	p.valueSet = true
}

func (p *parsingContext) isValueSet() bool {
	return p.valueSet
}
...
func (p parsingContext) objectPath() string {
	return p.key
}

func (p parsingContext) getPath() string {
	if p.inArray() {
		return p.arrayPath()
	}
	return p.objectPath()
